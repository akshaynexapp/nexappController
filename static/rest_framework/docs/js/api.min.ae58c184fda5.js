...
function normalizeKeys(arr){var _normarr=[];for(var i=0;i<arr.length;i++){_normarr=_normarr.concat(arr[i].split(' > '));}
return _normarr;}
function normalizeHTTPHeader(str){return(str.charAt(0).toUpperCase()+str.substring(1)).replace(/-(.)/g,function($1){return $1.toUpperCase()}).replace(/(Www)/g,function($1){return'WWW'}).replace(/(Xss)/g,function($1){return'XSS'}).replace(/(Md5)/g,function($1){return'MD5'})}
function formEntries(form){var formData=new FormData(form)
if(formData.entries!==undefined){return Array.from(formData.entries())}
var entries=[]
for(var i=0;i<form.elements.length;i++){var element=form.elements[i]
if(!element.name){continue}
if(element.type==='file'){for(var j=0;j<element.files.length;j++){entries.push([element.name,element.files[j]])}}else if(element.type==='select-multiple'||element.type==='select-one'){for(var j=0;j<element.selectedOptions.length;j++){entries.push([element.name,element.selectedOptions[j].value])}}else if(element.type==='checkbox'){if(element.checked){entries.push([element.name,element.value])}}else{entries.push([element.name,element.value])}}
return entries}
$(function(){var $selectedAuthentication=$('#selected-authentication')
...
$('#language-control li').click(function(event){event.preventDefault()
...
$('form.api-interaction').submit(function(event){event.preventDefault()
...
if(dataType==='integer'&&paramValue){var value=parseInt(paramValue)
if(!isNaN(value)){params[paramKey]=value}}else if(dataType==='number'&&paramValue){var value=parseFloat(paramValue)
if(!isNaN(value)){params[paramKey]=value}}else if(dataType==='boolean'&&paramValue){var value={'true':true,'false':false}[paramValue.toLowerCase()]
if(value!==undefined){params[paramKey]=value}}else if((dataType==='array'&&paramValue)||(dataType==='object'&&paramValue)){try{params[paramKey]=JSON.parse(paramValue)}catch(err){}}else if(dataType==='string'&&paramValue){params[paramKey]=paramValue}}
$form.find(':checkbox').each(function(index){var name=$(this).attr('name')
if(!params.hasOwnProperty(name)){params[name]=false}})
function requestCallback(request){var parser=document.createElement('a')
...
function responseCallback(response,responseText){$toggleView.removeClass('hide')
...
var options={requestCallback:requestCallback,responseCallback:responseCallback}
if(window.auth&&window.auth.type==='token'){options.auth=new coreapi.auth.TokenAuthentication({scheme:window.auth.scheme,token:window.auth.token})}else if(window.auth&&window.auth.type==='basic'){options.auth=new coreapi.auth.BasicAuthentication({username:window.auth.username,password:window.auth.password})}else if(window.auth&&window.auth.type==='session'){options.auth=new coreapi.auth.SessionAuthentication({csrfCookieName:'csrftoken',csrfHeaderName:'X-CSRFToken'})}
var client=new coreapi.Client(options)
client.action(schema,key,params).then(function(data){var response=JSON.stringify(data,null,2)
$requestAwaiting.addClass('hide')
$responseRaw.addClass('hide')
$responseData.addClass('hide').text('').jsonView(response)
if(responseDisplay==='data'){$responseData.removeClass('hide')}else{$responseRaw.removeClass('hide')}}).catch(function(error){var response=JSON.stringify(error.content,null,2)
$requestAwaiting.addClass('hide')
$responseRaw.addClass('hide')
$responseData.addClass('hide').text('').jsonView(response)
if(responseDisplay==='data'){$responseData.removeClass('hide')}else{$responseRaw.removeClass('hide')}})})
$('.toggle-view button').click(function(){var $modalContent=$(this).closest('.modal-content')
...
if(responseDisplay==='raw'){$modalResponseRaw.removeClass('hide')
$modalResponseData.addClass('hide')}else{$modalResponseData.removeClass('hide')
$modalResponseRaw.addClass('hide')}})
$authControl.find("[data-auth='none']").click(function(event){event.preventDefault()
window.auth=null
