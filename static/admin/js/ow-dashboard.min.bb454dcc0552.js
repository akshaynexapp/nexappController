(function () {
    'use strict'; function slugify(str) {
    ...
    const layout = {
        height: 315,
        width: 320,
        paper_bgcolor: 'rgba(0,0,0,0)', // Makes the chart's paper background transparent
        plot_bgcolor: 'rgba(0,0,0,0)',
        margin: { t: 0, b: 0 },
        legend: { yanchor: 'center', xanchor: 'left', x: 0, y: 0, bgcolor: 'transparent', font: { size: 14, family: 'Poppins ,Arial, sans-serif' } },
        title: { yanchor: 'center', y: 0.92, font: { size: 20, family: 'Poppins,Verdana, sans-serif' } }
    },
        options = { displayModeBar: false, };
    for (let i = 0; i < elementsParam.length; ++i) {
        layout.title.text = elementsParam[i].name;
        delete layout.annotations;
        let data = { type: 'pie', showlegend: !elementsParam[i].hasOwnProperty('quick_link') },
            element = document.createElement('div'), totalValues = 0;
        if (elementsParam[i].query_params.values.length === 0) {
            data.values = [1];
            data.labels = ['Not enough data'];
            data.marker = { colors: ['#adadad'] };
            data.texttemplate = ' ';
            ...
            if (elementsParam[i].colors) { data.marker = { colors: elementsParam[i].colors }; }
            ...
            for (var c = 0; c < data.values.length; c++) { totalValues += data.values[c]; }
        }
        ...
        if (elementsParam[i].query_params.values.length !== 0) {
            element.on('plotly_click', function (data) {
                var path = data.points[0].data.targetLink,
                    ...
                    i = data.points[0].i;
                if (filtering !== 'False') {
                    if (filters && typeof (filters[i]) !== 'undefined') { path += filters[i]; } else { path += encodeURIComponent(data.points[0].label); }
                }
