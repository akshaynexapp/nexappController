...
def deploy_site_to_site(tunnel_id, full_replace=False):
    ...
    print(">>> device_a_ip_network:",{tunnel.device_b_wan_ip})
    print(">>> payload_calls:", payload_calls)
    if tunnel.device_b_wan_ip and tunnel.device_a_wan_ip:

        if tunnel.last_config_hash and tunnel.last_config_payload:
            ConfigHistory.objects.create(
                tunnel=tunnel,
                config_hash=tunnel.last_config_hash,
                payload=tunnel.last_config_payload,
                action='push'
            )
        ...
        if all(r["api_code"] == 200 for r in results):
            ...
            tunnel.last_config_payload  = payload_calls
            tunnel.save()
